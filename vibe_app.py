...
# ================= MODELS =================
MODELS = [
    "https://router.huggingface.co/models/tiiuae/falcon-h1-1.5b-instruct",  # primary
    "https://router.huggingface.co/models/tiiuae/falcon3-1b-instruct",      # fallback
]

# ================= QUERY FUNCTION WITH FALLBACK =================
def query_hf(prompt):
...
with st.sidebar:
    ...
    for chat_id, chat_data in st.session_state.chats.items():
        if st.button(chat_data["title"], key=chat_id):
            ...
            st.rerun()
    st.markdown("---")
    st.caption("Using Hugging Face Falcon models with fallback")
...
if prompt:
    ...
    # Keep last 5 messages to save tokens
    conversation = ""
    for msg in current_chat["messages"][-5:]:
        role = "User" if msg["role"] == "user" else "Assistant"
        conversation += f"{role}: {msg['content']}\n"

    # Query HF Model with fallback
